...
namespace NWN2QuickItems.Utility
{
    ...
    public class ModEventHandler
    {
        ...
        public void Enable(string uniqueName, Assembly assembly)
        {
            if (Enabled)
            {
            ...
            using (ProcessLogger process = new ProcessLogger(Main.Logger))
            {
                try
                {
                    ...
                    if (!Patched)
                    {
                        Harmony harmonyInstance = new Harmony(uniqueName);
        ...
        public void Disable(string uniqueName, bool unpatch = false)
        {
            using (ProcessLogger process = new ProcessLogger(Main.Logger))
            {
                ...
                if (unpatch)
                {
                    ...
                    foreach (MethodBase method in harmonyInstance.GetPatchedMethods().ToList())
                    {
                        ...
                        IEnumerable<Patch> patches =
                            ...
                            .Where(patch => patch.owner == uniqueName);
                        if (patches.Any())
                        {
